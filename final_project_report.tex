...
We evaluated our final configurations via 5-fold cross-validation on the training set and a final evaluation on the SpamAssassin out-of-distribution test set, tracking Precision, Recall, Accuracy, and \(F_1\)-score (Table \ref{tab:results}).
...
As shown via the Precision-Recall curves in Figure \ref{fig:pr_comparison}, the Word2Vec model (Figure 5e) maintains the strongest precision across recall levels. The keyword baseline and TF-IDF unigram models (Figures 5a--5b) show rapid precision degradation at high recall, while the structurally-enriched SVM (Figure 5d) retains competitive precision-recall stability, granting administrators reliable margins when configuring production spam thresholds.
